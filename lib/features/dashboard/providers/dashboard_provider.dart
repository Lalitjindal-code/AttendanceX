  ...
  Stream<DashboardState> build() {
    ...
    return Rx.combineLatest4(
      ...
      (List<Schedule> schedules, List<Subject> subjects,
        ...
        final todaysAttendances = allAttendances
            .where((a) =>
                a.date.isAtSameMomentAs(todayUtc) ||
        ...
        final pendingLectures = todaysLectures
            .where((l) =>
                l.attendance == null ||
                l.attendance!.status == AttendanceStatus.pending)
            .toList();
        ...
        final todayProgressPercentage =
            totalClasses == 0 ? 0.0 : (markedCount / totalClasses);
        final todayProgressText = '$markedCount / $totalClasses Classes Marked';

        // Upcoming tasks
        final upcomingTasks =
            PlannerEngine.getDashboardUpcomingDeadlines(tasks);
        ...
        final startOfWeek =
            todayUtc.subtract(Duration(days: todayUtc.weekday - 1));
        ...
        for (final attendance in allAttendances) {
          ...
          final bool isPresent =
              attendance.status == AttendanceStatus.present ||
                  (attendance.status == AttendanceStatus.medical &&
                      settings.medicalCountsAsPresent) ||
                  (attendance.status == AttendanceStatus.gt &&
                      settings.gtMode == GtMode.countAsPresent);
          ...
          final bool isCounted =
              attendance.status == AttendanceStatus.present ||
                  ...
                  (attendance.status == AttendanceStatus.medical &&
                      settings.medicalCountsAsPresent) ||
                  (attendance.status == AttendanceStatus.gt &&
                      settings.gtMode != GtMode.exclude);

          if (!isCounted) continue;

          // Today
          if (attendance.date.year == todayUtc.year &&
              attendance.date.month == todayUtc.month &&
              attendance.date.day == todayUtc.day) {
            totalToday++;
