...
class PlannerEngine {
  ...
  static List<String> generateWeekendPlan(List<AcademicTask> pendingTasks) {
    if (pendingTasks.isEmpty) return ['You have no pending tasks. Enjoy your weekend!'];
      ...
      if (task.dueTime != null) {
        try {
          ...
          if (parts.length == 2) {
            ...
            dueDateTime = DateTime(dueDateTime.year, dueDateTime.month, dueDateTime.day, hour, minute);
          }
        } catch (_) {}
      } else {
        dueDateTime = DateTime(dueDateTime.year, dueDateTime.month, dueDateTime.day, 23, 59);
      }
      
      for (var offsetMinutes in task.notificationOffsets) {
        final scheduledTime = dueDateTime.subtract(Duration(minutes: offsetMinutes));
        
        if (scheduledTime.isAfter(now) && scheduledTime.difference(now).inDays < 30) {
