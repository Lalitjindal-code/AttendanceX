...
void main() {
  group('PlannerEngine Tests', () {
    ...
    test('generateDueLabel returns correct labels', () {
      ...
      final taskToday = AcademicTask()..dueDate = now..status = TaskStatus.pending;
      final taskTomorrow = AcademicTask()..dueDate = now.add(const Duration(days: 1))..status = TaskStatus.pending;
      final taskOverdue = AcademicTask()..dueDate = now.subtract(const Duration(days: 2))..status = TaskStatus.pending;
      final taskCompleted = AcademicTask()..dueDate = now..status = TaskStatus.completed;
    ...
    test('sortTasks prioritizes pending over completed, then by date, then priority', () {
      final now = DateTime.now();
      ...
      final tasks = [taskCompleted, taskPendingLate, taskPendingSoonLow, taskPendingSoonCritical];
