...
void main() {
  ...
  group('AnalyticsEngine.calculateTrend', () {
    test('Returns insufficientData when total is 0', () {
      ...
      expect(AnalyticsEngine.calculateTrend(current, prev),
          AnalyticsTrend.insufficientData);
      expect(AnalyticsEngine.calculateTrend(prev, current),
          AnalyticsTrend.insufficientData); // when prev is 0
    });
    ...
    test('Returns stable when percentage is same', () {
      ...
      expect(AnalyticsEngine.calculateTrend(summary, summary),
  ...
  group('AnalyticsEngine.calculateForecast', () {
    SubjectAttendanceSummary createSummary(int present, int total) {
      return SubjectAttendanceSummary(
        ...
        totalAbsentRecords: total - present,
        totalHolidayRecords: 0,
        totalMedicalRecords: 0,
        totalGTRecords: 0,
        totalPendingRecords: 0,
    ...
    test('Forecast logic math is correct', () {
      ...
      final forecast =
          AnalyticsEngine.calculateForecast(summary, 0.75); // goal 75%
